...
from src.library.bert.attention import AttentionLayer
...
class TransformerSelfAttentionLayer(Layer):
    ...
    def _construct(self, **kwargs):
        ...
        if params.hidden_size % params.num_heads != 0:
            raise ValueError(
                "The hidden_size:[{}] is not a multiple of num_heads:[{}]".
                format(params.hidden_size, params.num_heads))
    ...
    def call(self, inputs, mask=None, training=None, get_intermediate=False):
        layer_input = inputs

        #
        # TODO: is it OK to recompute the 3D attention mask
        # in each attention layer
        #
        intermediate_vals = {}
        attention_head, intermediate_val = self.attention_layer(
            layer_input,
            mask=mask,
            training=training,
        ...
        intermediate_vals["p"] = attention_output
        # modelのbuildの時にintermediate_valがNoneになるのでその対策
        if intermediate_val is not None:
            intermediate_vals.update(intermediate_val)
        if get_intermediate:
            return attention_output, intermediate_vals
        else:
...
class SingleTransformerEncoderLayer(Layer):
    ...
    def call(self, inputs, mask=None, training=None, get_intermediate=False):
        layer_input = inputs
        if get_intermediate:
            attention_output, intermediate_vals = self.self_attention_layer(
                layer_input,
                mask=mask,
                training=training,
                get_intermediate=get_intermediate)
        ...
        if get_intermediate:
            intermediate_vals["p_inter"] = intermediate_output
            return layer_output, intermediate_vals
        else:
...
class TransformerEncoderLayer(Layer):
    ...
    def call(self, inputs, mask=None, training=None, get_intermediate=False):
        ...
        for layer_ndx in range(self.params.num_layers):
            ...
            layer_output, intermediate_val = encoder_layer(
                layer_input,
                mask=mask,
                training=training,
                get_intermediate=get_intermediate)
            ...
            if get_intermediate:
                intermediate_vals.append(intermediate_val)
        ...
        if get_intermediate:
            return final_output, intermediate_vals
        else:
            return final_output
