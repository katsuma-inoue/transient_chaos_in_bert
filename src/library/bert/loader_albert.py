...
from src.library.bert import BertModelLayer, loader
...
def albert_params(albert_model: str):
    ...
    if tf.io.gfile.isdir(albert_model):
        config_file = os.path.join(
            albert_model, "assets",
            "albert_config.json")  # google tfhub v2 weights
        if not tf.io.gfile.exists(config_file):
            config_file = os.path.join(
                albert_model,
                "albert_config.json")  # google non-tfhub v2 weights
        ...
        if albert_model in albert_models_config:  # google tfhub v1 weights
            ...
            stock_config = loader.StockBertConfig.from_dict(
                albert_config, return_instance=True, return_unused=False)
        else:
            raise ValueError(
...
def fetch_google_albert_model(model_name: str, fetch_dir: str):
    if model_name not in albert_models_google:
        raise ValueError(
            "ALBERT model with name:[{}] not found at google-research/ALBERT, try one of:{}"
            .format(model_name, albert_models_google))
    else:
        fetch_url = albert_models_google[model_name]

    fetched_file = pf.utils.fetch_url(fetch_url, fetch_dir=fetch_dir)
    fetched_dir = pf.utils.unpack_archive(fetched_file)
    fetched_dir = tf.io.gfile.glob(
        os.path.join(fetched_dir, "*", "model.ckpt-best.meta"))[0]
...
def map_to_tfhub_albert_variable_name(name, prefix="bert"):
    ...
    name = name.replace(
        "encoder/layer_shared/output/LayerNorm",
        "encoder/transformer/group_0/inner_group_0/LayerNorm_1")
    name = name.replace(
        "encoder/layer_shared/attention",
...
def _is_tfhub_model(tfhub_model_path):
    try:
        assets_files = tf.io.gfile.glob(
            os.path.join(tfhub_model_path, "assets/*"))
...
def _is_google_model(ckpt_path):
    ...
    if ckpt_index:
        config_path = tf.io.gfile.glob(
            os.path.join(os.path.dirname(ckpt_path), "albert_config.json"))
        ckpt_meta = tf.io.gfile.glob(
            os.path.join(os.path.dirname(ckpt_path), "model.ckpt-best.meta"))
...
def load_albert_weights(bert: BertModelLayer, tfhub_model_path, tags=[]):
    ...
    if not _is_tfhub_model(tfhub_model_path):
        ...
        return loader.load_stock_weights(bert,
                                         tfhub_model_path,
                                         map_to_stock_fn=map_to_stock_fn)
    ...
    for ndx, (param_value, param) in enumerate(zip(param_values, bert_params)):
        ...
        if stock_name in stock_values:
            ...
            trace("loader: No value for:[{}], i.e.:[{}] in:[{}]".format(
